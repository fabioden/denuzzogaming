...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    const dprValue = dpr ?? (typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1);
    const renderer = new Renderer({ dpr: dprValue, alpha: true, antialias: true });
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      const x = (e.clientX - rect.left) * dprValue;
      const y = (rect.height - (e.clientY - rect.top)) * dprValue;
    ...
    return () => {
      ...
      const anyProg = programRef.current as unknown as { remove?: () => void } | null;
      const anyGeo = geometryRef.current as unknown as { remove?: () => void } | null;
      const anyMesh = meshRef.current as unknown as { remove?: () => void } | null;
      anyProg?.remove?.();
      anyGeo?.remove?.();
      anyMesh?.remove?.();
  ...
  return (
    <div
      ref={containerRef}
      className={className}
      style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", ...(mixBlendMode && { mixBlendMode }) }}
    />
  );
}
